...
## Governance Decisions — Why This Matters for Regulated Environments

Building this for a regulated context required deliberate architectural choices, not just working code.

**1. Human-in-the-loop by design**  
Every answer is generated from retrieved document chunks that are visible and auditable. A caseworker can see exactly which section of PS26/3 the answer came from. This satisfies the FCA's explainability requirement — the system can show its working.

**2. Confidence threshold requirement**  
During testing, the system returned similarity distance scores of 2.39 and 4.75 for a query about SMCR requirements. This exposed a critical production requirement: the system must refuse to answer when retrieval confidence is below a defined threshold, rather than returning a weakly-supported answer. In a regulated environment, this threshold is a **business decision**, not a technical one — and it belongs in the acceptance criteria, not the code comments.

**3. Dependency vulnerability identified and mitigated**  
The default ChromaDB embedding package (`@chroma-core/default-embed`) introduced 3 high severity CVEs via its `sharp`/`libvips` image processing dependency chain. This was identified during build, the package was rejected, and an alternative embedding approach was implemented. In a bank security review, this dependency chain would fail immediately. Documenting this decision is part of the governance trail.

**4. Production upgrade path documented**  
Current embeddings use a simplified approach suitable for prototyping. Production deployment would replace this with a dedicated embedding model (OpenAI `text-embedding-3-small` or Azure OpenAI embeddings) for accurate semantic retrieval. This is a one-function swap — the rest of the architecture is production-ready.

---

## Test Results — Ground Truth Evaluation

Three questions were tested against the system. All three returned accurate, grounded answers.
...
This prototype demonstrates the core RAG pattern. A production version for a motor finance redress programme would add:
...
- **Multi-document ingestion** — ingest the full PS26/3 alongside DISP complaint handling rules and Consumer Duty guidance
